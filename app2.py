...
# --- START: NEW KANNADA NUMBER CONVERSION FUNCTIONS ---
ones_kn = {
...
def convert_less_than_hundred_kn(n):
    if n < 10: return ones_kn[n]
    if n < 20: return teens_kn[n]
    tens = (n // 10) * 10
    ones = n % 10
    if ones == 0: return tens_kn[tens]
    return tens_prefixes_kn[tens] + ones_kn[ones]

def convert_integer_part_kn(n):
    if n == 0: return ones_kn[0]
    if n < 100: return convert_less_than_hundred_kn(n)
    
    parts = []
    if n >= 10000000:
        parts.append(f"{convert_integer_part_kn(n // 10000000)} ಕೋಟಿ")
        n %= 10000000
    if n >= 100000:
        parts.append(f"{convert_less_than_hundred_kn(n // 100000)} ಲಕ್ಷ")
        n %= 100000
    if n >= 1000:
        parts.append(f"{convert_less_than_hundred_kn(n // 1000)} ಸಾವಿರ")
        n %= 1000
    if n >= 100:
        hundreds = n // 100
        # Special handling for "ನೂರು" (100) vs "ಇನ್ನೂರು" (200) etc.
        parts.append("ನೂರು" if hundreds == 1 else f"{ones_kn[hundreds]} ನೂರು")
        n %= 100
    if n > 0:
        parts.append(convert_less_than_hundred_kn(n))
    return ' '.join(parts)

def number_to_kannada_words(num):
    """Converts a float to its Kannada word representation."""
    ...
    integer_words = convert_integer_part_kn(integer_part)
    
    # Process decimal part only if it's not zero
    if int(decimal_part_str) == 0:
        return integer_words

    decimal_words = ' '.join(ones_kn[int(digit)] for digit in decimal_part_str)
    
    return f"{integer_words} ಪಾಯಿಂಟ್ {decimal_words}"
# --- END: NEW KANNADA NUMBER CONVERSION FUNCTIONS ---
...
def get_recommendation_route():
    ...
    if lang == 'kn':
        ...
        # --- MODIFICATION START: Convert price to Kannada words ---
        price_kannada = number_to_kannada_words(price)
        response_text = f"ನಿಮ್ಮ {district_kannada} ಜಿಲ್ಲೆಗೆ, ಅತ್ಯಂತ ಲಾಭದಾಯಕ ಬೆಳೆ {crop_kannada} ಆಗಿದೆ, ಅದರ ಕೊಯ್ಲು ಬೆಲೆ ಪ್ರತಿ ಕೆಜಿಗೆ {price_kannada} ರೂಪಾಯಿ ಎಂದು ಅಂದಾಜಿಸಲಾಗಿದೆ."
        # --- MODIFICATION END ---
